...
**Adversarial review of this sync** (Fable subagent, full transcript not retained) diffed the fork's net contribution pre- vs post-rebase (`b963f4ad..backup/pre-rebase-20260828` vs `upstream/master..master`) across all 17 sensitive files and found them **byte-identical** apart from the intended `cache_policy.py`/`cli_args.py` change and two cosmetic blank lines in `utils.py` — i.e. the replay dropped nothing. It also mutation-tested the RAM-pressure regression test (removed the `_outputs_contain_model_patcher` guard → the test correctly failed) confirming the new monkeypatch seam is not vacuous, and re-verified `CoreModelPatcher is ModelPatcher` with the reassignment still confined to `main.py`'s aimdo block. It surfaced **one real defect**, now fixed: the streaming loader's header validation lacked upstream's new `_SAFETENSORS_MAX_HEADER_SIZE` cap, so it would accept (and pread whole) a 100MB–filesize header that `safe_open` rejects as "HeaderTooLarge" — a silent divergence between the two load paths, and it falsified the existing comment claiming the fallback "rejects it identically". Fixed in `fix(loader): enforce safetensors header-size cap …` with a mutation-verified regression test (`test_bounds_check_rejects_oversized_header`). **Lesson for future syncs: when upstream tightens validation on a path the fork has forked around, check whether the fork's parallel implementation needs the same tightening** — the streamer re-implements header parsing, so it does not inherit `safe_open`'s checks for free.
