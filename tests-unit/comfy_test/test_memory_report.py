...
class TestMemoryReport:
    def test_format_with_cuda_available(self, monkeypatch):
        """When CUDA is available, format should be 'alloc X.XG res X.XG free X.XG | sys free X.XG avail X.XG used X.XG'."""
        ...
        assert result == "alloc 45.2G res 46.1G free 71.3G | sys free 60.0G avail 74.8G used 52.9G"

    def test_format_without_cuda(self, monkeypatch):
        """When CUDA unavailable, format should be 'sys free X.XG avail X.XG used X.XG'."""
        monkeypatch.setattr("torch.cuda.is_available", lambda: False)

        mock_vm = MagicMock()
        mock_vm.free = int(100.0 * 1024**3)
        mock_vm.available = int(118.2 * 1024**3)
        mock_vm.used = int(9.3 * 1024**3)
        monkeypatch.setattr("psutil.virtual_memory", lambda: mock_vm)

        result = mm.memory_report()
        assert result == "sys free 100.0G avail 118.2G used 9.3G"
