...
- `custom_nodes/Comfy_HunyuanImage3/` is a separate fork (origin: `EricRollei/Comfy_HunyuanImage3`). The Spark page-cache-management commit (`3ae4cb9`) was dropped 2026-05-02 because upstream's restructured `HunyuanInstructLoader` rejected all 10 hunks of a 3-way patch attempt. Re-implementation is pending verification that the original symptom (~83GB OS page cache buildup during HunyuanImage3 load on unified memory) still reproduces against current upstream. Snapshot, original patch, and verification notes live in `dev-docs/spark-port-pending/`.
