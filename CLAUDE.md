...
- Unified memory gotcha: `ModelPatcher.patch_weight_to_device` backs up original weights for `unpatch_model`. The default `weight.to(offload_device)` backup is a full second allocation in the same pool — large LoRA toggles (e.g. Flux2.dev) will OOM. When `should_assign_weights()` is True and `inplace_update` is False, store the original `weight` in `self.backup` by reference (`set_attr_param` rebinds, so the original tensor stays valid). Any new patch-time tensor allocation must branch on `should_assign_weights()` the same way the initial load path does.
