...
- Sync with upstream: `git fetch upstream && git rebase upstream/master`. Before any rebase, snapshot HEAD: `git branch backup/pre-rebase-$(date +%Y%m%d) HEAD && git tag pre-rebase-$(date +%Y%m%d) HEAD`. Expected conflict: `comfy/model_patcher.py::patch_weight_to_device` if upstream changes its signature again — keep the fork's invertible-LoRA branching in `force_load_param` and thread any new upstream parameters (e.g., `force_cast=True`) through the `self.patch_weight_to_device(...)` call. The fork's stack was rebased clean (8 logical commits) on 2026-05-02; the personal-config commit (CLAUDE.md, dev-docs, transformers pin) lives at the tip so future rebases can drop or amend it independently. See `upstream_sync.md` for the operational checklist.
